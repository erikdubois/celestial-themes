...
bump_version() {
    ...
    # Split PKGBUILD: pkgname is an array, so report the pkgbase instead.
    pkgname=$(grep -m1 -E '^(pkgbase|pkgname)=' "${pkgbuild}" | cut -d= -f2)
...
build_package() {
    ...
    # Only the recipe goes to the build dir. This repo is ~900 MB of rendered
    # themes and the PKGBUILD fetches them again from git, so copying the whole
    # folder would burn a second gigabyte in /tmp for nothing.
    [[ -d /tmp/tempbuild ]] && rm -rf /tmp/tempbuild
    mkdir /tmp/tempbuild
    cp "${pkgbuild}" /tmp/tempbuild/
    ...
    if [[ "${success}" == "true" ]]; then
        ...
        # One package per colour family, so the files are named
        # celestial-aliz-*, celestial-aqua-*, ... — never celestial-themes-*.
        # Match on the extension and take everything the build produced.
        local -a built=()
        mapfile -t built < <(find /tmp/tempbuild -maxdepth 1 -name '*.pkg.tar.zst' -printf '%f\n' | sort)

        if [[ "${#built[@]}" -eq 0 ]]; then
            log_error "No packages found in /tmp/tempbuild"
            exit 1
        fi

        install -dm755 "${destiny}"
        cp -nv /tmp/tempbuild/*.pkg.tar.zst "${destiny}" || \
            log_warn "Some packages already exist in ${destiny} — those were skipped"

        log_info "Built ${#built[@]} packages for ${search}"
        printf "%s\n" "${search}" "${built[@]}" | tee -a /tmp/installed
    fi

    log_section "Cleaning up"
    find "${SCRIPT_DIR}" -maxdepth 1 \( -name "*.log" -o -name "*.deb" -o -name "*.tar.gz" \) -delete

    # Keep the build dir when something went wrong — the chroot logs live there.
    if [[ "${success}" == "true" ]]; then
        rm -rf /tmp/tempbuild
    fi
